    ...
    public class When_base_event_from_2_publishers : NServiceBusAcceptanceTest
    {
        ...
        public class Publisher1 : EndpointConfigurationBuilder
        {
            public Publisher1()
            {
                ...
                {
                    context.AddTrace($"{Conventions.EndpointNamingConvention(typeof(Publisher1))} SubscriberEndpoint={s.SubscriberReturnAddress}");
        ...
        public class Publisher2 : EndpointConfigurationBuilder
        {
            public Publisher2()
            {
                EndpointSetup<DefaultPublisher>(b => b.OnEndpointSubscribed<Context>((s, context) =>
                {
                    context.AddTrace($"{Conventions.EndpointNamingConvention(typeof(Publisher2))} SubscriberEndpoint={s.SubscriberReturnAddress}");
