namespace NServiceBus.Persistence.AzureStorage.Config
{
    ...
    class ThrowIfNoCloudTableClientForSubscriptionsProvider : IProvideCloudTableClientForSubscriptions
    {
        public CloudTableClient Client => throw new Exception(
            $"No CloudTableClient has been configured. Either provide a connection string, use `persistence.UseCloudTableClient(client)` or register an implementation of `{nameof(IProvideCloudTableClient)}` in the container.");
    }
}